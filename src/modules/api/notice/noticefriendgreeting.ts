...
export default class extends ApiAction {
  ...
  public async execute() {
    const mails = await this.connection.query(`SELECT * FROM user_greet WHERE receiver_id = :user AND deleted_from_receiver = 0 ORDER BY notice_id DESC`, {
      user: this.user_id
    })
    ...
    await mails.forEachAsync(async (mail: any) => {
      ...
      list.push({
        ...
        affector: {
          ...
          center_unit_info: await this.user.getCenterUnitInfo(mail.affector_id),
