...
export default class {
  ...
  public async execute() {
    ...
    for (const deck of this.params.unit_deck_list) {
      ...
      assert(deck.unit_deck_detail.length === 9, "unit_deck_detail != 9")

      let usedPositions: number[] = []
      let usedUnitIds: number[] = []
    ...
    let check = await this.connection.query(`SELECT unit_owning_user_id FROM units WHERE user_id=:user AND deleted=0 AND unit_owning_user_id IN (${allUnits.join(",")})`, { user: this.user_id })
    assert(check.length === allUnits.length, "User doesn't have this cards")

    let insertDeck = []
    let insertSlot = []
    for (const deck of decks) {
      let name = deck.deck_name.replace(/['"«»]/g, "'")
